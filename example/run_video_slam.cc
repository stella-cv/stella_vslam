...
void mono_tracking(const std::shared_ptr<stella_vslam::config>& cfg,
                   ...
                   const bool eval_log, const std::string& map_db_path, const bool visualize) {
    // load the mask image
...
// and pass the frame_publisher and the map_publisher
    ...
    std::thread thread([&]() {
        while (is_not_end) {
            ...
            ++num_frame;
            // std::cout<<total_frames<<","<<num_frame<<std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (cfg->camera_->setup_type_ == stella_vslam::camera::setup_type_t::Monocular) {
        mono_tracking(cfg, vocab_file_path->value(), video_file_path->value(), mask_img_path->value(),
                      ...
                      eval_log->is_set(), map_db_path->value(), visualize_pangolin->value());
    }
    else {
